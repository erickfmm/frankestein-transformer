...
Sections 2 and 3 describe the configuration contract and the supported model families. Section 4 explains normalization options exposed by the current schema. Section 5 covers optimizer routing and training dynamics. Sections 6 and 7 describe deployment and SBERT workflows. Appendix A--D act as annexes summarizing the supporting literature files in \texttt{docs/bibliography/}.

\begin{figure}[t]
\centering
\fitfigure{\begin{tikzpicture}[node distance=1.7cm and 0.9cm]
\node[box, fill=blue!8] (cli) {\textbf{CLI Surface}\\\texttt{train}, \texttt{infer}, \texttt{deploy}\\\texttt{sbert-train}, \texttt{sbert-infer}};
\node[box, fill=green!8, below=of cli] (schema) {\textbf{Schema Contract}\\\texttt{model\_class}, \texttt{model}, \texttt{training}};
\node[box, fill=yellow!12, below left=of schema, xshift=-1.5cm] (mixers) {\textbf{Model Families}\\standard, sparse, gated,\\retentive, SSM, ODE, memory};
\node[box, fill=orange!12, below=of schema] (optim) {\textbf{Optimizer Router}\\prefixed hyperparameters\\and safety controls};
\node[box, fill=red!8, below right=of schema, xshift=1.5cm] (deploy) {\textbf{Outputs}\\quantized checkpoints,\\embeddings, inference artifacts};
\draw[line] (cli) -- (schema);
\draw[line] (schema) -- (mixers);
\draw[line] (schema) -- (optim);
\draw[line] (schema) -- (deploy);
\draw[line] (mixers) -- (deploy);
\draw[line] (optim) -- (deploy);
\end{tikzpicture}}
\caption{System view of the project: the CLI dispatches into a strict schema, which then controls architecture selection, optimizer behavior, and deploy/inference outputs.}
