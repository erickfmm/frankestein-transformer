...
KDA & 2025 & Matrix (recurrent) & Channel-wise decay + scalar write & $\mathcal{O}(Ld^2)$ & $\mathcal{O}(d^2)$ memory & \citep{kimi_kda_2025} \\
\bottomrule
\end{longtable}
\normalsize
...
KDA & Yes & Very strong & Very strong & Excellent & Channel-wise decay; deployed in Kimi Linear (75\% KV reduction, $6\times$ decode) \\
\bottomrule
\end{longtable}
\normalsize
...
KDA & High (up to $6\times$ decode at 1M) & $O(d^2)$ constant & Production hybrid (Kimi Linear); long-context decode \\
\bottomrule
\end{longtable}
\normalsize
